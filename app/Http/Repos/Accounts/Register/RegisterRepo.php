...
class RegisterRepo implements RegisterInterface
{
    ...
    public function store($request)
    {
        ...
        $cachedlocation = Redis::get('location_'.$request->address1.$request->city.$request->country);
        if(isset($cachedlocation)) {
            ...
            $location = $this->location($request->address1, $request->city, $request->country);
            ...
            if ($location['status'] == 200) {
                ...
                Redis::set('location_'.$request->address1.$request->city.$request->country, json_encode(['lat' => $cords['lat'], 'lng' => $cords['lng']]));
            } else {
